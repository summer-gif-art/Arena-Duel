...
public class PlayerXP : MonoBehaviour
{
    ...
    public float survivalTickInterval = 3f; // Earn XP every N seconds alive
    ...
    public void AddXP(int amount, string source)
    {
        ...
        // While loop prevents skipping levels on large XP gains
    ...
    public void OnHitEnemy() => AddXP(xpPerHit, "Hit");

    // Called from PlayerCombat when blocking
    public void OnSuccessfulBlock() => AddXP(xpPerBlock, "Block");

    // Called from PlayerCombat when dodging
    public void OnSuccessfulDodge() => AddXP(xpPerDodge, "Dodge");

    void OnEnemyKilled() => AddXP(xpPerKill, "Kill");

    void OnPlayerDied() => isAlive = false;
